...
function calculateCircleArea(radius) {
  ...
  let cirle = Math.PI * Math.pow(radius, 2);
  return cirle;
...
function roundObjectValues(obj) {
  ...
  let newMap = entries.map(([key, value]) => {
  ...
  let newEntries = Object.fromEntries(newMap);
...
function calculateVolumeCylinder(radius, height) {
  ...
  let obCil = Math.PI * Math.pow(radius, 2) * height;

  return Math.ceil(obCil);
...
function sumPositiveNumbers(numbers) {
  ...
  for (let i = 0; i <= numbers.length; i++) {
    if (Math.sign(numbers[i]) === 1 || Math.sign(numbers[i]) === 0) {
      sum += numbers[i];
    }
  }
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let totalPrice = products.reduce((acc, value) => {
    return acc + value[1];
  }, 0);
